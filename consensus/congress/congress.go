...
var (
	...
	etherPrecision5 = big.NewInt(685350000000000000)
)
...
func (c *Congress) Prepare(chain consensus.ChainHeaderReader, header *types.Header) error {
	...
	if err != nil {
		log.Error("Prepare", "snapshot", err.Error())
		return err
	}
	...
	if number%c.config.Epoch == 0 {
		...
		if err != nil {
			log.Error("Prepare", "getTopValidators", err.Error())
			return err
		}
...
func (c *Congress) trySendBlockRewardV2(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB) error {
	number := header.Number.Uint64()
...
func (c *Congress) getTopValidators(chain consensus.ChainHeaderReader, header *types.Header) ([]common.Address, error) {
	...
	msg := vmcaller.NewLegacyMessage(header.Coinbase, systemcontract.GetValidatorAddr(header.Number, c.chainConfig), 0, new(big.Int), math.MaxUint64, new(big.Int), data, false)
	...
	// unpack data
	ret, err := c.abi[systemcontract.GetValidatorsContractName(header.Number)].Unpack(method, result)
	if err != nil {
		return []common.Address{}, err
	}
	if len(ret) != 1 {
...
func (c *Congress) IsSysTransaction(sender common.Address, tx *types.Transaction, header *types.Header) (bool, error) {
	...
	if sender == header.Coinbase && *to == systemcontract.SysGovToAddr {
		return true, nil
	}
	return false, nil
}
...
func (c *Congress) CanCreate(state consensus.StateReader, addr common.Address, height *big.Int) bool {
	if c.chainConfig.IsRedCoast(height) && c.config.EnableDevVerification {
		if isDeveloperVerificationEnabled(state) {
			...
			valueHash := state.GetState(systemcontract.SysGovToAddr, slot)
